...
class SignUpUseCase(
    ...
    fun execute(signUpRequestDto: SignUpRequestDto) {
        if(queryUserPort.existsByEmail(signUpRequestDto.email))
            throw AlreadyExistsUserException()
        val encodePassword = securityService.encodePassword(signUpRequestDto.password)
        val user = signUpRequestDto.toEntity(encodePassword)
