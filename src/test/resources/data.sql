...
drop table if exists global_env_detail_entity cascade;
...
create table global_env_entity (id BINARY(16) not null, description varchar(255), name varchar(255), workspace_id BINARY(16), primary key (id));
create table global_env_detail_entity (id BINARY(16) not null, encryption bit not null, env_key varchar(255), env_value varchar(255), env_id BINARY(16), primary key (id));
...
alter table if exists global_env_detail_entity add constraint FK3d2gmm4ofwrslih24mxyhf71y foreign key (env_id) references global_env_entity (id);
alter table if exists global_env_entity add constraint FK694b15y5r7k158yn9c2rnftte foreign key (workspace_id) references workspace_entity (id);
