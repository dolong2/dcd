    ...
    given("연결할 도메인이 주어지고") {
        ...
        `when`("도메인에 애플리케이션을 연결할때") {
            ...
            then("Nginx 설정 파일을 생성해야함") {
                ...
                val httpConfigFilePath = "${httpConfigDirectory}/${application.name.replace(" ", "-")}-http.conf"
                verify { fileOperationPort.createDirectory(match { it.toString() == httpConfigDirectory }) }
                verify { fileOperationPort.writeFile(match { it.toString() == httpConfigFilePath }, webServerConfig) }
            }
