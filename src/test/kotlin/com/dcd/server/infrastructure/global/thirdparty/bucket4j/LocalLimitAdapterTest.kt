...
@SpringBootTest
@ActiveProfiles("test")
class LocalLimitAdapterTest(
    private val localLimitAdapter: LocalLimitAdapter
) : BehaviorSpec({
    given("테스트할 Limit 어노테이션이 주어지고") {
        ...
        `when`("어노테이션에 설정된 횟수 초과로 토큰을 소비할때") {
            ...
            for (i in 0 until capacity) {
                localLimitAdapter.consumer(targetLimitAnnotation.target, targetLimitAnnotation)
            }

            then("반환값이 false여야함") {
                localLimitAdapter.consumer(targetLimitAnnotation.target, targetLimitAnnotation) shouldBe false
            }
        }
    }
})